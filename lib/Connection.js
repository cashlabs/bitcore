...
function Connection(socket, peer, opts) {
  ...
  this.network = networks[this.config.network.name];
  if (this.network === undefined) {
	  console.log('[bitcore/Connection.js:32]: no network..');
	  process.exit(-1);
  }
  if (this.network.name === 'dogetest' || this.network.name === 'dogelive') {
    PROTOCOL_VERSION = 70003;
    console.log('[bitcore/Connection.js]: set PROTOCOL_VERSION ' +
      PROTOCOL_VERSION + ' for network ' + this.network.name);
  } else if (this.network.name === 'ltctest' || this.network.name === 'ltclive' ) {
