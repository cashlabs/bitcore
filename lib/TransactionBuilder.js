...
TransactionBuilder.infoForP2sh = function(opts, networkName) {
  ...
  for(var x in networks){
	if(networkName == networks[x].name ){
		  version = networks[x].P2SHVersion;
  	};
 }
...
TransactionBuilder.prototype._multiFindKey = function(walletKeyMap, pubKeyHash) {
  var wk;
//  [ networks.lienet, networks.tstnet].forEach(function(n) {
//  networks.forEach(function(n) {
  for(var x in networks){
    [networks[x].addressVersion, networks[x].P2SHVersion].forEach(function(v) {
