...
var HierarchicalKey = function(bytes) {
  ...
  for (var x in networks) {
      if(bytes === networks[x].name) {
          isnetname=1;
          this.version = networks[x].hkeyPrivateVersion;
      }
  }
  if (isnetname) {
    this.depth = 0x00;
...
HierarchicalKey.seed = function(bytes, network) {
	 if (!network) {
		  console.log('[bitcore/BIP32.js:67]: no network..');
		  process.exit(-1);
	  }
...
HierarchicalKey.prototype.initFromBytes = function(bytes) {
  ...
  var isPublic = 0;
  for(var x in networks){
	if(this.version == networks[x].hkeyPrivateVersion ){
		  isPrivate = 1;
  	}
	if(this.version == networks[x].hkeyPublicVersion ){
		  isPublic = 1;
	}
 }
...
HierarchicalKey.prototype.buildExtendedPublicKey = function() {
  ...
  var v = null;
  for (var x in networks) {
      if (this.version===networks[x].hkeyPublicVersion || this.version===networks[x].hkeyPrivateVersion) {
