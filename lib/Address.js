...
Address.fromPubKey = function(pubKey, network) {
  if (!network) {
	  console.log('[bitcore/Address.js:18]: no network..');
	  process.exit(-1);
  }
...
Address.fromPubKeys = function(mReq, pubKeys, network, opts) {
	  if (!network) {
		  console.log('[bitcore/Address.js:34]: no network..');
		  process.exit(-1);
	  }
...
Address.fromScript = function(script, network) {
	  if (!network) {
		  console.log('[bitcore/Address.js:52]: no network..');
		  process.exit(-1);
	  }
...
Address.fromScriptPubKey = function(scriptPubKey, network) {
  ...
  if (!network) {
	  console.log('[bitcore/Address.js:78]: no network..');
	  process.exit(-1);
  }
...
Address.prototype.network = function() {
  ...
  for(var x in networks){
	if(version === networks[x].addressVersion || version === networks[x].P2SHVersion ){
		  answer = networks[x];
  	}
  };
...
Address.prototype.getScriptPubKey = function() {
  ...
  for(var x in networks){
	  if(version === networks[x].addressVersion){
    script = Script.createPubKeyHashOut(this.payload());
	  } else if (version === networks[x].P2SHVersion){
